...
f. At the trailing edge where $B$ is very small, $A$ is very close to $1$,
...
We concentrate on the second equation, which we  solve with the Ansatz $B(x)=\exp(\lambda x)$, where we need $\lambda$ to be real and negative to match the shape of $B$ at the trailing edge.
