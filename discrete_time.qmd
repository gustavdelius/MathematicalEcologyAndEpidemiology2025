...
point but will be evolving towards it. @fig-cobweb-positive-slope shows the cobweb for a stable fixed point with positive slope.
...
of a small change in a model parameter, for example the intrinsic growth rate. For example,
@fig-period-doubling-bifurcation shows the graph of $f(N)$ of the Verhulst model at
...
plt.xlabel("$N$")
plt.ylabel(r"$f(N)=RN(1-N/\tilde{K})$")
plt.show()
```
